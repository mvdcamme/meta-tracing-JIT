...
;;;; Traced recursive Slip interpreter with annotations for measuring trace duplicationbut WITHOUT
;;;; annotations for merging traces. This interpreter should be executed by the tracing interpreter.
  ...
  (define (loop output)
    ...
    (define (close parameters expressions closure-name)
      ...
      (define (closure . arguments)
        ...
        (displayln "after can-start-loop")
        ...
        (let* ((value (evaluate-sequence expressions)))
          ...
          (displayln "after can-close-loop")
          value))
      closure)
