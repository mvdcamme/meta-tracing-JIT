      ...
      (let ((total-number-of-traces-metric-name "total-number-of-traces")
        ...
        (define (run-trace-duplication-metric)
          (let* ((trace-duplication-metric-result (calculate-trace-duplication))
                 (root-expression (car trace-duplication-metric-result))
                 (all-ast-nodes (cadr trace-duplication-metric-result)))
            (define (calculate-average-trace-duplication)
              (define (loop lst sum length)
                (if (null? lst)
                    (/ sum length)
                    (loop (cdr lst)
                          (+ sum
                             (vector-ref (car lst) 1))
                          (+ length (if (> (vector-ref (car lst) 1) 0)
                                        1
                                        0)))))
              (loop all-ast-nodes 0.0 0))
            (output-metric trace-duplication-metric-name (calculate-average-trace-duplication))))
