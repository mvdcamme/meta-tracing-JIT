  ...
  (define (run-trace-metrics)
    (let ((total-number-of-traces-metric-name "total-number-of-traces")
      ...
      (define (run-total-number-of-traces-metric)
        (output-metric total-number-of-traces-metric-name (calculate-total-number-of-traces GLOBAL_TRACER_CONTEXT)))
      (define (run-average-trace-length-metric)
        (output-metric average-trace-length-metric-name (calculate-average-trace-length GLOBAL_TRACER_CONTEXT)))
      (define (run-trace-executions-metric)
        ...
        (show-times-executed (get-trace-executions GLOBAL_TRACER_CONTEXT))
        (output-newline))
