...
(struct ratork (xs ρ)) ;'continue-after-operator'
...
(struct apply-nat-fun (n)) ;apply a native function to the first n values from the list of saved values
  ...
  (state-tracer-context
   (match state
     ...
     ((ev `(,rator . ,rands) ρ σ κ)
      (ev rator ρ σ (cons (ratork rands ρ) κ)))
     ...
     ((ap rator rands ρ σ (cons φ κ))
      (set! tracer-context (add-trace-transition tracer-context (apply-nat-fun (length rands))))
     ...
     ((ko (ratork rands ρ) v ρ* σ κ)
      (and (clo? v)
           (set! tracer-context (add-trace-transition tracer-context (rem-val)))) ;the evaluated operator should not be stored as a value
      (ev (car rands) ρ σ (cons (randk (cdr rands) (list v) ρ) κ)))
...
(define (step-trace s instruction)
  (match instruction
    ...
    ((apply-nat-fun n)
     (let* ((vals (reverse (take (traced-state-values s) (+ n 1))))
            (nat-fun (car vals))
            (operands (cdr vals)))
       (print nat-fun)
       (print operands)
       (let ((result (apply nat-fun operands)))
         (struct-copy traced-state s (values (cons result (drop (traced-state-values s) (+ n 1))))))))
    ((put-val val)
