  ...
  (define (tree-rec el)
    (cond ((pair? el)
           (cond ((eq? (car el) 'define)
                 ...
                 ((eq? (car el) 'lambda)
                  (t 'lambda (cons (cadr el) (map tree-rec (cddr el))) 0))
                 ((eq? (car el) 'let*)
                  (t 'let* 
                 ...
                 ((or (eq? (car el) 'and)
                      ...
                      (eq? (car el) 'or)
                      (eq? (car el) 'set!))
                  (t (car el) (map tree-rec (cdr el)) 0))
...
(define (step state)
  (and (is-tracing?)
       (ev? state)
       (cond ((t? (ev-e state)) (set-t-counter! (ev-e state) (+ (t-counter (ev-e state)) 1)))
             ((u? (ev-e state)) (set-u-counter! (ev-e state) (+ (u-counter (ev-e state)) 1)))))
  (match state
    ...
    ((ko (condk pes es) κ)
     (execute `(restore-env))
     (if v
         (begin (execute `(guard-true ,(inc-guard-id!) ',(t 'cond es 0)))
                (eval-seq pes κ))
         (begin (execute `(guard-false ,(inc-guard-id!) ',(t 'begin pes 0)))
                (ev (t 'cond es 0) κ))))
