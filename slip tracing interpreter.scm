...
(define (step* s)
  (match s
    ...
    ((ev `(can-start-loop ,e  . ,debug-info) κ)
     ...
     (cond ((expression-traced? global-tracer-context e)
            (display "----------- EXECUTING TRACE -----------") (newline)
            (let ((trace (expression-trace global-tracer-context e)))
              (let loop ()
                (run-trace trace)
                (loop))))
           ((is-tracing-expression? global-tracer-context e)
           ...
           ((not (is-tracing? global-tracer-context))
            ...
            (clear-trace!)
            (set! global-tracer-context (start-tracing-expression global-tracer-context e))
            (let ((new-state (step (ev e κ))))
              (step* new-state)))
           (else
