(module tracing-interpreter racket
  ...
  (struct condk (pes es labels) #:transparent)
  (struct definevk (x)) ;define variable
  (struct haltk ())
  (struct ifk (e1 e2))
  (struct letk (x es))
  (struct let*k (x bds es labels))
  (struct letreck (x bds es labels))
  (struct ork (es labels))
    ...
    (define (tree-rec el)
      (cond ((pair? el)
             (cond ((eq? (car el) 'define)
                   ...
                   ((eq? (car el) 'quote)
                    (t 'quote (cadr el) '()))
                   ((eq? (car el) 'quasiquote)
                    (t 'quasiquote (cadr el) '()))
                   ((eq? (car el) 'unquote)
                    (t 'unquote (cadr el) '()))
                   ((or (eq? (car el) 'and)
                        (eq? (car el) 'apply)
  ...
  (define (step state)
    (and (is-tracing?)
         ...
         (let ((trace-key (tracer-context-trace-key GLOBAL_TRACER_CONTEXT)))
           (cond ((t? (ev-e state))
                  (and (not (member-equal? trace-key (t-counter (ev-e state))))
                       (set-t-counter! (ev-e state) (cons trace-key (t-counter (ev-e state))))))
                 ((u? (ev-e state))
                  (and (not (member-equal? trace-key (u-counter (ev-e state))))
                       (set-u-counter! (ev-e state) (cons trace-key (u-counter (ev-e state)))))))))
    (match state
      ((ev (t 'and `(,e . ,es) labels) κ)
       (execute `(add-continuation ,(andk es labels)))
       (ev e (cons (andk es labels) κ)))
      ...
      ((ev (t 'let  `(((,var ,val) . ,bds) . ,es) labels) κ)
       (and (not (null? bds))
      ...
      ((ev (t 'let* `(((,var ,val) . ,bds) . ,es) labels) κ)
       (execute `(save-env)
                `(add-continuation ,(let*k var bds es labels)))
       (ev val (cons (let*k var bds es labels) κ)))
      ((ev (t 'letrec `(((,x ,e) . ,bds) ,es ...) labels) κ)
       (execute `(literal-value '())
                `(alloc-var ',x)
                `(save-env)
                `(add-continuation ,(letreck x bds es labels)))
       (ev e (cons (letreck x bds es labels) κ)))
      ((ev (t 'or `(,e . ,es) labels) κ)
       (execute `(add-continuation ,(ork es labels)))
       (ev e (cons (ork es labels) κ)))
      ...
      ((ko (andk es labels) κ)
       (if v
           (begin (execute `(add-continuation ,(andk (cdr es) labels)))
                  (ev (car es) (cons (andk (cdr es) labels) κ)))
      ...
      ((ko (condk pes '() _) κ)
       (execute `(restore-env))
       (if v
           (begin (execute `(guard-true ,(inc-guard-id!) (u '() '())))
                  (eval-seq pes κ))
      ...
      ((ko (condk pes `(,(t pred pred-es _) . ,es) labels) κ)
       ...
       (if v
           ...
           (begin (execute `(guard-false ,(inc-guard-id!) ',(t 'begin pes '()))
                           `(save-env)
                           `(add-continuation ,(condk pred-es es labels)))
                  (ev pred (cons (condk pred-es es labels) κ)))))
      ...
      ((ko (let*k x '() es _) κ)
       (execute `(restore-env)
                `(alloc-var ',x))
       (eval-seq es κ))
      ((ko (let*k x `((,var ,val) . ,bds) es labels) κ)
       (execute `(restore-env)
                `(alloc-var ',x)
                `(save-env)
                `(add-continuation ,(let*k var bds es labels)))
      ...
      ((ko (letreck x '() es _) κ)
       (execute `(restore-env)
                `(set-var ',x))
       (eval-seq es κ))
      ((ko (letreck x `((,var ,val) . ,bds) es labels) κ)
       (execute `(restore-env)
                `(set-var ',x)
                `(alloc-var ',var)
                `(save-env)
                `(add-continuation ,(letreck var bds es labels)))
       (ev val (cons (letreck var bds es labels) κ)))
      ((ko (ork '() _) κ)
       (execute `(remove-continuation))
       (ko (car κ) (cdr κ)))
      ((ko (ork es labels) κ)
  ...
  (define (step* s)
    (match s
      ((ko (haltk) _)
       v)
      ((ev (t (u 'splits-control-flow _) `() _) (cons φ κ))
       (execute `(remove-continuation)
