(module trace-graph racket
  ...
  (define (make-label-trace-info directory-path file-name)
    (let* ((label-function-name (get-label-function-name-from-file-name file-name))
           (label-id-string (get-label-id-from-file-name file-name))
           (label-node-name (create-label-node-name label-function-name label-id-string)))
      (label-trace-info (string->number label-id-string) label-node-name)))
  ...
  (define (make-graph-file directory-path)
    (let* ((trace-files (get-trace-files-in-directory directory-path))
      ...
      (define (handle-label-trace-file trace-file-path)
        (let* ((label-function-name (get-label-function-name-from-file-name trace-file-path))
               (label-id-string (get-label-id-from-file-name trace-file-path))
               (label-node-name (create-label-node-name label-function-name label-id-string)))
          (add-node-to-graph-file directory-path label-node-name)
          (when (label-trace-loops? (string-append directory-path "\\" trace-file-path))
            (add-edge-to-graph-file directory-path label-node-name label-node-name))
          (handle-trace-file trace-file-path label-node-name)))
