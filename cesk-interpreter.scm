(module cesk-interpreter racket
  ...
  (define (guard-same-closure program-state clo i guard-id)
    (when (not (clo-equal? (program-state-v program-state) clo))
      (output "Closure guard failed, expected: ") (output clo) (output ", evaluated: ") (output (program-state-v program-state)) (output-newline)
